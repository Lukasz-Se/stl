...
bool isItCorrectNumber(std::string input) {
    ...
    if (!std::all_of(begin(input), end(input),
                     [all](char c) { return std::any_of(begin(all), end(all), [c](char special) { return c == special || isdigit(c); }); }))
        return false;
    return true;
}
...
ErrorCode CheckBadChars(std::string* input, const std::string& symbols) {
    if (!std::all_of(begin(*input), end(*input), [symbols](char c) { return std::any_of(begin(symbols), end(symbols), [c](char aa) { return c == aa || isdigit(c); }); }))
